(async () => {
  ...
  function montarTarefas() {
    ...
    const autoRespondidas = progresso.get(`AUTO:${user.idUsuario}`)?.size || 0;
    tarefas.push({
      titulo: `Autoavaliação • ${cicloAtual.nome}`,
      tipo: 'AUTO',
      idAvaliado: user.idUsuario,
      nomeAvaliado: user.nome,
      prazo: API.Util.formatarData(cicloAtual.dataFim),
      respondidas: autoRespondidas,
      total: totalSoftskills,
    });

    colegas.forEach(colega => {
      const respondidas = progresso.get(`360:${colega.idUsuario}`)?.size || 0;
      tarefas.push({
        titulo: `360° • ${colega.nome}`,
        tipo: '360',
  ...
  function renderizarTarefas() {
    if (!tarefas.length) {
      tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#888;padding:1rem">Nenhuma avaliação disponível.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = tarefas.map((tarefa, index) => {
      ...
      return `
        <tr>
          ...
          <td><span class="emblema ${tarefa.tipo === 'AUTO' ? 'nota-verde' : 'nota-azul'}">${API.Util.escaparHtml(API.Util.labelTipo(tarefa.tipo))}</span></td>
  ...
  function preencherKpis() {
    ...
    document.getElementById('statConcluidoDesc').textContent = `${totalConcluido} de ${totalEsperado} respostas enviadas`;
  }

  function popularAvaliados(tarefa = null) {
    const tipo = tarefa?.tipo || selectTipo.value || 'AUTO';
    selectTipo.value = tipo;

    const opcoes = tipo === 'AUTO'
      ? [{ idUsuario: user.idUsuario, nome: `${user.nome} (você)` }]
      : colegas;

    selectAvaliado.innerHTML = opcoes.map(item => `
